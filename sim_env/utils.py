...
class RobotModelLoader:
    ...
    def __init__(self, robot_name: str, **kwargs) -> None:
        self.robot_name = robot_name

        # Optional arguments
        mj_scene_path = os.path.join(os.path.dirname(__file__), RobotModelLoader.DEFAULT_SCENE_NAME)
        self.description_file_path = os.path.join(os.path.dirname(__file__), RobotModelLoader.DEFAULT_DESCRIPTION_NAME)
        ...
        if self.robot_dir != "":
            ...
            self.path_urdf, self.path_mjcf, self.package_dir = self._get_robot_model_paths_from_repo(self.robot_name)
    ...
    def _get_xml_string(self) -> str:
        ...
        if 'meshdir=""' in xml_string:
            ...
            xml_string = xml_string.replace('meshdir=""', f'meshdir="{abs_path_mesh_dir}"')
