...
class PinQuadRobotWrapper(AbstractQuadRobotWrapper):
    ...
    def _init_actuator_map(self) -> dict[str, int]:
        ...
        joint_id2name = {i: name for name, i in self.joint_name2id.items()}
        # Map to joints
        joint_name2act_id = {name: i for i, name in enumerate(joint_id2name.values())}
    ...
    def update_contacts(self) -> bool:
        ...
        if not self.contact_updated:
            ...
            for collision_pair, result in zip(
                self.collision_model.geometryObjects,
                self.collision_data.activeCollisionPairs,
            ):
                if result:
