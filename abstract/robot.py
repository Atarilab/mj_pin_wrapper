...
class AbstractQuadRobotWrapper(AbstractRobotWrapper):
    ...
    BASE_NAME = "base"
    JOINT_NAMES = HIP_NAMES + THIGH_NAMES + CALF_NAMES
    ...
    def _is_description_valid(self) -> bool:
        ...
        """
        if self.nq == 19 and self.nv == 18 and self.nu == 12 and self.ne == 4:
            return True
    ...
    def foot_contacts(self) -> dict[str, int]:
        ...
        for cnt_pair in self.contacts:
            if cnt_pair[0] in self.static_geom_id and cnt_pair[1] in self.eeff_idx:
                eeff_name = self.eeff_id2name[cnt_pair[1]]
                foot_contacts[eeff_name] = cnt_pair[0] + 1

            elif cnt_pair[1] in self.static_geom_id and cnt_pair[0] in self.eeff_idx:
